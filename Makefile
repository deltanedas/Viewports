...
build: Viewports-Desktop.jar
android: build Viewports.jar
...
Viewports-Desktop.jar: $(classes) $(assets)
	@printf "\033[33m> JAR\033[0m\t%s\n" $@
	jar -cf $@ $(JARFLAGS) || rm $@

Viewports.jar: Viewports-Desktop.jar
	@printf "\033[33m> D8\033[0m\t%s\n" $@
	$(D8) $(D8FLAGS) --output build $^
	cp Viewports-Desktop.jar $@
	cd build; zip -qg ../$@ classes.dex

install: build
	cp Viewports-Desktop.jar $(MINDUSTRY)/mods
